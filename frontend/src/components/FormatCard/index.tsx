...
const FormatCard = ({format, title, url}: FormatProps) => {
    ...
    const formatedPath = `/audio/download?encodedURI=${encodeURIComponent(url)}&ext=${(format as IFormat).ext}&title=${formatedTitle}&formatId=${(format as IFormat).format_id}`    
    ...
    const handleClick = async (e: React.MouseEvent<HTMLButtonElement>) => {
        console.log(types[(format as IFormat).ext], "OPTIONS")
        console.log(formatedPath, "PATH")
        const data = await request(formatedPath, {"Content-Type": `${types[(format as IFormat).ext]}`})
        console.log(data)
    }
    ...
    return(
        <div className='FormatCard-container'>
            ...
            <button className='download-btn' onClick={(e : React.MouseEvent<HTMLButtonElement>) => handleClick(e)}>Download</button>
